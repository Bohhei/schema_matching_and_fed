...
import pandas as pd
import torch
from torch.utils.data import Dataset as Dataset_1
...
class Dataset(Dataset_1):
    def __init__(self, fname, tokenizer,dat_fname):
        ...
        for i in range(reader.shape[0]):
            ...
            constraints = reader.iloc[i][5]
            new_constr = []
            for c in constraints:
                if type(c)==list:
                    for j in c:
                        try:
                            new_constr.append(float(j))
                        except:
                            print('Need encoder')
                else:
                    new_constr.append(float(c))
            ...
            data = {
                ...
                'class_n': int(class_n),
                'constraints': torch.tensor(new_constr, dtype=torch.float32)
            }
            all_data.append(data)
            
            self.all_data_reverce[reader.iloc[i][2]] = text_raw_indices1
            self.all_data_reverce[reader.iloc[i][3]] =text_raw_indices2
        self.data = all_data

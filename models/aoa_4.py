...
class AOA_4(nn.Module):
    def __init__(self, embedding_matrix, opt):
        super(AOA_4, self).__init__()
        self.opt = opt
        self.fc_dim = 512
        ...
        self.lin1_c = nn.Linear(int(self.opt.constr_dim/2), 64)
    ...
    def forward(self, inputs):
        ...
        half_ = int(self.opt.constr_dim/2)
        constr = list([torch.abs(i[:half_]-i[half_:]) for i in constraints])
        constraints = torch.cat(constr, dim = 0).reshape(-1,half_)
        ...
        out_1_c = self.bn1_c(self.lin1_c(constraints))

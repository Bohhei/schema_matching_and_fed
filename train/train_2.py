...
class Instructor:
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader, lrshrink, minlr):
        ...
        while not stop_training and epoch <= self.opt.num_epoch:
            ...
            if report_val['class1']['f1-score'] > max_val_acc:
                ...
                path = 'state_dict_2/'+self.opt.dataset+'/{0}_{1}_val_f1_{2}'.format(self.opt.model_name, self.opt.dataset, round(report_val['class1']['f1-score'], 4))
    ...
    def _train_cross_val(self, criterion, optimizer, dataset, lrshrink, minlr, number):
        max_val_acc = 0
        max_val_f1 = 0
        global_step = 0
        path = None
        ...
        while not stop_training and epoch <= self.opt.num_epoch:
            ...
            for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(len(dataset)))):
                ...
                train_loader = DataLoader(train_set, batch_size=self.opt.batch_size, sampler=sampler)
            ...
            logger.info('> val_acc: {:.4f}, val_f1: {:.4f}'.format(np.mean(total_val_acc), 
                                                                   np.mean(total_val_f1)))
            #logger.info('> report: {}'.format(report_val['class1']))
            print('timepassed:')
            logger.info('> timepassed: {}'.format((time.time()-ts)))
            ...
            if np.mean(total_val_f1) > max_val_f1:
                max_val_f1 = np.mean(total_val_f1)

                if not os.path.exists('state_dict_2/'+self.opt.dataset):
                    os.makedirs('state_dict_2/'+self.opt.dataset)
                path = 'state_dict_2/'+self.opt.dataset+'/{0}_{1}_val_f1_{2}'.format(self.opt.model_name, self.opt.dataset+number, round(max_val_f1, 4))
    ...
    def run(self, number=''):
        ...
        if self.opt.K_Fold!='true':
            ...
            self._reset_params()
            best_model_path = self._train_cross_val(criterion, optimizer, self.trainset, self.opt.lrshrink, self.opt.minlr, number)
...
def main():
    ...
    parser.add_argument('--model_name', default='aoa_4', type=str, help = 'aoa for AOA, abl for Ablation, sela for selfattention')
    ...
    parser.add_argument('--num_epoch', default=10, type=int, help='default 20')

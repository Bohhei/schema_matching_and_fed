...
class AOA_2(nn.Module):
    def __init__(self, embedding_matrix, opt):
        super(AOA_2, self).__init__()
        self.opt = opt
        self.fc_dim = 512
        ...
        #self.constr_dim = opt.constr_dim*2
        ...
        #self.lin2_2 =  nn.Linear(self.fc_dim, 8)
        #self.bn2 = BatchNorm1d(8)
        self.lin3 = nn.Linear(self.fc_dim, opt.class_dim)

        '''
        self.lin1_c = nn.Linear(self.constr_dim+8, self.constr_dim+8)
        self.bn1_c = BatchNorm1d(self.constr_dim+8)
        self.lin2_c = nn.Linear(self.constr_dim+8, opt.class_dim)
        '''
        '''
        #для constraints
        self.lin1_c = nn.Linear(self.constr_dim, self.fc_dim//2)
        self.bn1_c = BatchNorm1d(self.fc_dim//2)
        self.lin2_c = nn.Linear(self.fc_dim//2, self.fc_dim)
        self.bn2_c = BatchNorm1d(self.fc_dim)

        self.obtained = nn.Linear(self.fc_dim*2, opt.class_dim)
        '''
    ...
    def forward(self, inputs):
        ...
        out_1 = self.bn1(F.relu(self.lin1(features)))
        out_2 = self.dropout(out_1)
        out_3 = self.bn1(F.relu(self.lin2(out_2)))
        #out = self.bn2(F.relu(self.lin2_2(out_3)))
        out_4 = self.lin3(out_3)
        '''
        input_c = torch.cat((constraints, out), 1)
        out_1_c = self.bn1_c(F.relu(self.lin1_c(input_c)))
        out_2_c = self.lin2_c(out_1_c)
        '''
        
        '''
        constraints = self.flatten(constraints)
        out_1_c = self.bn1_c(F.relu(self.lin1_c(constraints)))
        out_2_c = self.bn2_c(F.relu(self.lin2_c(out_1_c)))
        
        obtained_outs = torch.cat((out_3, out_2_c), dim=1)
        out_4 = self.obtained(obtained_outs)
        '''

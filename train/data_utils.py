...
class Dataset(Dataset):
    def __init__(self, fname, tokenizer,dat_fname):
        if os.path.exists(dat_fname):
            ...
            tokenizer1 = SentencePieceBPETokenizer(vocab, merges)
            name = dat_fname.split('_')[0]
            print(name)
            if name=='num' or name =='nom' or name=='tpc':
                reader = pd.read_pickle(fname)
            else:
                reader = pd.read_excel(fname)
            all_data = []
...
class Dataset_2(Dataset):
    def __init__(self, fname, embeddings = []):
        if len(embeddings)>0:
            reader = pd.read_pickle(fname)
            c = reader.columns
            constr = reader[c[5]].values
            labels = reader[c[4]].values
            new_constr = []
            for ind in range(len(constr)):
                constr_ind = constr[ind]
                new_constr.append([])
                for i in [0,1]:
                    for j in constr_ind[i]:
                        try:
                            new_constr[ind].append(float(j))
                        except:
                            print('Need encoder')
            with open(fname+'embeddings.p', 'wb') as f:
                pickle.dump(embeddings, f)
            with open(fname+'constraints.p', 'wb') as f:
                pickle.dump(new_constr, f)
        else:
            reader = pd.read_pickle(fname)
            c = reader.columns
            labels = reader[c[4]].values
            with open(fname+'embeddings.p', 'rb') as f:
                embeddings = pickle.load(f)
            with open(fname+'constraints.p', 'rb') as f:
                new_constr = pickle.load(f)
        
        
        
        self.constr_len = len(new_constr[0])
        self.emb_len = len(embeddings[0])
        
        self.data_X = torch.tensor([[new_constr[i] + list(embeddings[i])] for i in range(len(embeddings))], dtype= torch.float32)
        self.data_Y = torch.tensor(labels)
        
    def get_len_sample(self):
        return self.constr_len + self.emb_len
